...
export class SmartTime extends LitElement {
  ...
  niceTimestamp() {
    return new Intl.DateTimeFormat(navigator.language, {
      dateStyle: "short",
      timeStyle: this.timestamp ? "short" : undefined,
    }).format(this.t);
  }
  ...
  fullTimestamp() {
    return new Intl.DateTimeFormat(navigator.language, {
      dateStyle: "full",
      timeStyle: this.hoverTimestamp || this.timestamp ? "full" : undefined,
    }).format(this.t);
  }

  render() {
    return html`
      <time datetime="${this.timestamp}" title="${this.fullTimestamp()}">
        ${this.niceTimestamp()}
      </time>
    `;
  }
